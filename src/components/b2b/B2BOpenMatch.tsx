...
const B2BOpenMatch = () => {
  return (
    <section className="py-16 md:py-24 bg-gradient-to-b from-b2b-background to-b2b-background-secondary">
      <div className="container mx-auto px-4">
        ...
        <div className="relative mb-16">
          ...
          <div className="hidden lg:block absolute top-16 left-[12.5%] right-[12.5%] h-0.5 bg-gradient-to-r from-b2b-primary/20 via-b2b-primary to-b2b-primary/20" />
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-4">
            {steps.map((step, index) => (
              <div key={index} className="relative">
                <div className="bg-b2b-background-secondary border border-b2b-border rounded-2xl p-6 h-full hover:border-b2b-primary/50 transition-colors">
                  {/* Step number with icon */}
                  <div className="flex items-center gap-4 mb-4">
                    <div className="relative">
                      <div className="w-14 h-14 rounded-xl bg-b2b-primary/10 flex items-center justify-center">
                        <step.icon className="w-7 h-7 text-b2b-primary" />
                      </div>
                      <span className="absolute -top-2 -right-2 w-6 h-6 bg-b2b-primary text-white text-xs font-bold rounded-full flex items-center justify-center">
                        {step.number}
                      </span>
                    </div>
                  </div>
                ...
                {index < steps.length - 1 && (
                  <div className="lg:hidden flex justify-center py-2">
                    <div className="w-0.5 h-6 bg-b2b-primary/30" />
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
            {results.map((result, index) => (
              <div key={index} className="text-center">
                <div className="w-12 h-12 mx-auto mb-3 rounded-xl bg-b2b-primary/10 flex items-center justify-center">
                  <result.icon className="w-6 h-6 text-b2b-primary" />
                </div>
                <div className="text-2xl md:text-3xl font-bold text-b2b-primary mb-1">
                  {result.value}
                </div>
